...
Background = zeros(height, width);
  ...
  NGauss = size(GMM.mean,3);
    ...
    for m=1:NGauss
        u_diff(:,:,m) = distance2Gauss(currentFrameGray, GMM.mean(:,:,m));
    end
    ...
    for i=1:height
        for j=1:width
            ...
            for k=1:NGauss 
                ...
                if (belongs2Gauss(u_diff(i,j,k), GMM.sd(i,j,k), GMM.param.D)) % pixel matches component
            ...
            while ((match == 0)&&(k<=GMM.param.M))
                ...
                if (GMM.w(i,j,rank_ind(k)) >= GMM.param.thresh)
                    if (belongs2Gauss(u_diff(i,j,rank_ind(k)), GMM.sd(i,j,rank_ind(k)), GMM.param.D))
                        Foreground(i,j) = 0;
                        match = 1;
                    else
