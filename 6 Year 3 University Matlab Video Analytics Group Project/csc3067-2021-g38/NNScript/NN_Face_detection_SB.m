...
modelNN = NNtraining_JC(Training_Images, Training_Labels);

save('./Variables/modelNN.mat', 'modelNN');

%Timing=toc
%saveToText("NN training", Timing);
...
for i=1:size(Testing_Images,1)
    ...
    classificationResult(i,1) = NNTesting_SB(testnumber, modelNN);
...
%Timing=toc
%saveToText("NN test dataset ", Timing, Accuracy, TP, FP, TN, FN);
...
for i=1:7
    ...
    prediction = NNTesting_SB(testnumber, modelNN);
...
%Timing=toc;
%saveToText("NN supplied images ", Timing, Accuracy, TP, FP, TN, FN);
...
for i=1:4
    ...
    for y=1:samplingY:size(currentImage,1)
        for x=1:samplingX:size(currentImage,2)
            ...
            if (x+FaceSize(2)-1 <= size(currentImage,2))&&(y+FaceSize(1) <= size(currentImage,1))
                ...
                [prediction, confidence] = NNTesting_WC_SB(possibleFace,modelNN);
                if prediction==1
    ...
    %Timing=toc;
    %saveToText("NN single scale sliding " + i, Timing, Accuracy, TP, FP, TN, FN);
end
...
for i=1:4
    ...
    Accuracy = (TN+TP)/WindowIterations
    %Timing=toc;
    %saveToText("NN multi scale sliding " + i, Timing, Accuracy, TP, FP, TN, FN);
end