...
[Training_Images, Training_Labels] = DCTLoadFaceImagesV2('./Resources/Provided/face_train.cdataset');
...
modelSVMDCT = SVMtraining(Training_Images, Training_Labels);

%Timing=toc
%saveToText("SVM DCT training ", Timing);

save('./Variables/modelSVMDCT.mat', 'modelSVMDCT');


%% Basic Testing With Dataset
%Load testing Images
[Testing_Images, Testing_Labels] = DCTLoadFaceImagesV2('./Resources/Provided/face_test.cdataset');
...
for i=1:size(Testing_Images,1)
    ...
    [prediction, score] = SVMTesting(testnumber, modelSVMDCT);
...
%Timing=toc
%saveToText("SVM DCT test dataset ", Timing, Accuracy, TP, FP, TN, FN, areaUnderCurve);
...
for i = 1:7
   ...
   faces(i,:) = DCT_Feature_VectorV2(tmpIm);
end
...
for i=1:7
    ...
    prediction = SVMTesting(testnumber, modelSVMDCT);
...
%Timing=toc;
%saveToText("SVM DCT supplied images ", Timing, Accuracy, TP, FP, TN, FN);
...
for i=1:4
    ...
    for y=1:samplingY:size(currentImage,1)
        for x=1:samplingX:size(currentImage,2)
            ...
            if (x+FaceSize(2)-1 <= size(currentImage,2))&&(y+FaceSize(1) <= size(currentImage,1))
                ...
                possibleFace = DCT_Feature_VectorV2(possibleFace);
                [prediction, confidence] = SVMTesting(possibleFace,modelSVMDCT);
                if prediction==1
    ...
    Timing=toc;
    saveToText("SVM DCT single scale sliding " + i, Timing, Accuracy, TP, FP, TN, FN);
end
...
prediction=-1;
predictedRow = [];
FaceSizes = [27,18; 22,15; 31,21;];% 18,12; 36,24;];
intersectionThreshold = 0.2;
im1RectArray=[];
im2RectArray=[];
im3RectArray=[];
...
for i=1:4
    ...
    Timing=toc;
    saveToText("SVM DCT multi scale sliding ", Timing, Accuracy, TP, FP, TN, FN);
end