...
Timing=toc
saveToText("SVM HOG training ", Timing);
...
Timing=toc
saveToText("SVM HOG test dataset ", Timing, Accuracy, TP, FP, TN, FN, areaUnderCurve);
...
for i=1:7
    ...
    prediction = SVMTesting(testnumber, modelSVMHOG);
...
Timing=toc;
saveToText("SVM HOG supplied images ", Timing, Accuracy, TP, FP, TN, FN);
...
for i=1:4
    ...
    for y=1:samplingY:size(currentImage,1)
        for x=1:samplingX:size(currentImage,2)
            ...
            if (x+FaceSize(2)-1 <= size(currentImage,2))&&(y+FaceSize(1) <= size(currentImage,1))
                ...
                possibleFace = extractHOGFeatures(possibleFace);
                [prediction, confidence] = SVMTesting(possibleFace,modelSVMHOG);
                if prediction==1
    ...
    Timing=toc;
    saveToText("SVM HOG single scale sliding " + i, Timing, Accuracy, TP, FP, TN, FN);
end
...
for i=1:4
    ...
    Timing=toc;
    saveToText("SVM HOG multi scale sliding ", Timing, Accuracy, TP, FP, TN, FN);
end
