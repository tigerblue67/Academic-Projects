...
% Create a mask for our convolution
Mask(1:7,1:7) = 1/49;
...
for i=1:size(Training_Images, 3)
    Training_Images(:, :, i) = filter2(Mask, Training_Images(:, :, i));
...
%Timing=toc
%saveToText("SVM CONV training ", Timing);
...
for i=1:size(Testing_Images, 3)
    Testing_Images(:, :, i) = filter2(Mask, Testing_Images(:, :, i));
...
%Timing=toc
%saveToText("SVM CONV test dataset ", Timing, Accuracy, TP, FP, TN, FN);
...
%Timing=toc;
%saveToText("SVM CONV supplied images ", Timing, Accuracy, TP, FP, TN, FN);