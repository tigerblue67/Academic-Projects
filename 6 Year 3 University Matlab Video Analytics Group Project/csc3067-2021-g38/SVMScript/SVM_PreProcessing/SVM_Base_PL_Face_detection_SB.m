...
for i=1:size(Training_Images, 3)
    % Gamma value can be changed here (2nd argument)
    Training_Images(:, :, i) = enhanceContrastPL(Training_Images(:, :, i), 1.25);
...
% Timing=toc
% saveToText("SVM PL training ", Timing);
...
for i=1:size(Testing_Images, 3)
    Testing_Images(:, :, i) = enhanceContrastPL(Testing_Images(:, :, i), 1.25);
...
% Timing=toc
% saveToText("SVM PL test dataset ", Timing, Accuracy, TP, FP, TN, FN);
...
for i = 1:7
   ...
   % figure
   % imshow(tmpIm);
   tmpIm = enhanceContrastPL(tmpIm, 1.25);
...
% Timing=toc;
% saveToText("SVM PL supplied images ", Timing, Accuracy, TP, FP, TN, FN);
