...
nTrees=500;
modelRF = TreeBagger(nTrees,Training_Images,Training_Labels, 'Method', 'classification', 'OOBPrediction', 'On'); 
%Timing=toc
%saveToText("RF PCA training", Timing);

% Was used for investigation into number of trees
figure('Name', 'Random Forrest Tree Investigation');
h=plot(oobError(modelRF));
...
%Timing=toc
%saveToText("RF PCA test dataset", Timing, Accuracy, TP, FP, TN, FN, areaUnderCurve);
...
%Timing=toc;
%saveToText("RF PCA supplied images ", Timing, Accuracy, TP, FP, TN, FN);
