...
modelKNNHOG = NNtraining_JC(Training_Images, Training_Labels);

save('./Variables/modelKNNHOG.mat', 'modelKNNHOG');

%Timing=toc
%saveToText("HOG KNN training", Timing);
...
for i=1:size(Testing_Images,1)
    ...
    classificationResult(i,1) = KNNTesting_JC(testnumber, modelKNNHOG, 5);
...
%Timing=toc;
%saveToText("HOG KNN test dataset ", Timing, Accuracy, TP, FP, TN, FN);
...
for i=1:7
    ...
    prediction = KNNTesting_JC(testnumber, modelKNNHOG, 5);
...
%Timing=toc;
%saveToText("HOG KNN supplied images ", Timing, Accuracy, TP, FP, TN, FN);
