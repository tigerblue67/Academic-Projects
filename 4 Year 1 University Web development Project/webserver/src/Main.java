...
import views.FAQ;
import views.KimKardashianInglotMakeup;
import views.NewsPage;
import views.Index;
import views.WinterFashion;
import views.NYFW;
import views.AW18;
import views.NavBar_inc_NewsStory;
import views.ChrisEvansHairstyle;
import views.HipsterLook;
import views.SummerLook;
import views.JeffWingerStyle;
import views.DwightSchruteLook;
import views.Component;
//import views.Navbar;
//import views.FooterPagePlusCodeForDatabaseSubmission;
import views.NewsletterPage;
import views.OutfitPage;
...
public class Main 
{
	...
	{		
		...
		registerShutdownHook(databaseInterface);		System.out.println("Project:csc24");

		GetTheLook iGetTheLook = new GetTheLook(databaseInterface,fileStoreInterface);

		MakeYourLook iMakeYourLook = new MakeYourLook(databaseInterface,fileStoreInterface);

		ContactUs iContactUs = new ContactUs(databaseInterface,fileStoreInterface);

		SignIn iSignIn = new SignIn(databaseInterface,fileStoreInterface);

		AboutUs iAboutUs = new AboutUs(databaseInterface,fileStoreInterface);

		Register iRegister = new Register(databaseInterface,fileStoreInterface);

		FAQ iFAQ = new FAQ(databaseInterface,fileStoreInterface);

		KimKardashianInglotMakeup iKimKardashianInglotMakeup = new KimKardashianInglotMakeup(databaseInterface,fileStoreInterface);

		NewsPage iNewsPage = new NewsPage(databaseInterface,fileStoreInterface);

		Index iIndex = new Index(databaseInterface,fileStoreInterface);

		WinterFashion iWinterFashion = new WinterFashion(databaseInterface,fileStoreInterface);

		NYFW iNYFW = new NYFW(databaseInterface,fileStoreInterface);

		AW18 iAW18 = new AW18(databaseInterface,fileStoreInterface);

		NavBar_inc_NewsStory iNavBar_inc_NewsStory = new NavBar_inc_NewsStory(databaseInterface,fileStoreInterface);

		ChrisEvansHairstyle iChrisEvansHairstyle = new ChrisEvansHairstyle(databaseInterface,fileStoreInterface);

		HipsterLook iHipsterLook = new HipsterLook(databaseInterface,fileStoreInterface);

		SummerLook iSummerLook = new SummerLook(databaseInterface,fileStoreInterface);

		JeffWingerStyle iJeffWingerStyle = new JeffWingerStyle(databaseInterface,fileStoreInterface);

		DwightSchruteLook iDwightSchruteLook = new DwightSchruteLook(databaseInterface,fileStoreInterface);

		Component iComponent = new Component(databaseInterface,fileStoreInterface);

		//Navbar iNavbar = new Navbar(databaseInterface,fileStoreInterface);

		//FooterPagePlusCodeForDatabaseSubmission iFooterPagePlusCodeForDatabaseSubmission = new FooterPagePlusCodeForDatabaseSubmission(databaseInterface,fileStoreInterface);

		NewsletterPage iNewsletterPage = new NewsletterPage(databaseInterface,fileStoreInterface);

		OutfitPage iOutfitPage = new OutfitPage(databaseInterface,fileStoreInterface);

		Template iTemplateLook = new Template(databaseInterface,fileStoreInterface);

		Page iPage = new Page(databaseInterface,fileStoreInterface);
			...
			for (int i = 0; i < requestToProcess.size(); i++) 
			{
		        ...
		        //the example dynamic page will process the request if the index.html file is not present
		        	if(toProcess.path.length()==0)
		        		toProcess.path = "index.html";		if(iGetTheLook.process(toProcess))
		{
		}
		else
		if(iMakeYourLook.process(toProcess))
		{
		}
		else
		if(iContactUs.process(toProcess))
		{
		}
		else
		if(iSignIn.process(toProcess))
		{
		}
		else
		if(iAboutUs.process(toProcess))
		{
		}
		else
		if(iRegister.process(toProcess))
		{
		}
		else
		if(iFAQ.process(toProcess))
		{
		}
		else
		if(iKimKardashianInglotMakeup.process(toProcess))
		{
		}
		else
		if(iNewsPage.process(toProcess))
		{
		}
		else
		...
		if(iWinterFashion.process(toProcess))
		{
		}
		else
		if(iNYFW.process(toProcess))
		{
		}
		else
		if(iAW18.process(toProcess))
		{
		}
		else
		if(iNavBar_inc_NewsStory.process(toProcess))
		{
		}
		else
		if(iChrisEvansHairstyle.process(toProcess))
		{
		}
		else
		if(iHipsterLook.process(toProcess))
		{
		}
		else
		if(iSummerLook.process(toProcess))
		{
		}
		else
		if(iJeffWingerStyle.process(toProcess))
		{
		}
		else
		if(iDwightSchruteLook.process(toProcess))
		{
		}
		else
		if(iComponent.process(toProcess))
		{
		}

		else
		if(iNewsletterPage.process(toProcess))
		{
		}
		else
		if(iOutfitPage.process(toProcess))
		{
		}
		else
		if(iTemplateLook.process(toProcess))
		{
		}
		else
		if(iPage.process(toProcess))
		{
		}
		else
		        {
