...
public class WebRequest implements Runnable
{
	...
	public void run()
	{
		try
		{
			...
			{
				sendResponse( r.status, r.location, r.mimeType, r.header, r.data, r.range );
				in.close();
			}
		}
	...
	{
		sendResponse( status, null, WebResponse.MIME_PLAINTEXT, null, new ByteArrayInputStream( msg.getBytes() ), -1);
	 ...
	 */
	private void sendResponse( String status, String location, String mime, HashMap<String,String> header, InputStream data, long range )
	{
		try
		{
			if ( status == null )
			...
			System.out.println("The webserver has replied with a "+status+" status and a "+mime+" type of data");
			...
			if( (mime!=null)&&(mime.equalsIgnoreCase(WebResponse.MIME_HTML)) )
			{
//				pw.flush();
			}
