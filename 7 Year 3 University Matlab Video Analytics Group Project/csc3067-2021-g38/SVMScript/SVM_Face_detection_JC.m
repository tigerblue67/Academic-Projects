...
%Timing=toc
%saveToText("SVM training ", Timing);
...
%Timing=toc
%saveToText("SVM test dataset ", Timing, Accuracy, TP, FP, TN, FN, areaUnderCurve);
...
%Timing=toc;
%saveToText("SVM supplied images ", Timing, Accuracy, TP, FP, TN, FN);
...
for i=1:4
    ...
    for y=1:samplingY:size(currentImage,1)
        for x=1:samplingX:size(currentImage,2)
            ...
            if (x+FaceSize(2)-1 <= size(currentImage,2))&&(y+FaceSize(1) <= size(currentImage,1))
                ...
                [prediction, confidence] = SVMTesting(possibleFace,modelSVM);
                if prediction==1
    ...
    %Timing=toc;
    %saveToText("SVM single scale sliding " + i, Timing, Accuracy, TP, FP, TN, FN);
end
