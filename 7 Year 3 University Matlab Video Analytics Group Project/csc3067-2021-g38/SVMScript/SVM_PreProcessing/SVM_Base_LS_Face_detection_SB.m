...
for i=1:size(Training_Images, 3)
    Training_Images(:, :, i) = enhanceContrastAuto(Training_Images(:, :, i));
...
%Timing=toc
%saveToText("SVM LS training ", Timing);
...
for i=1:size(Testing_Images, 3)
    Testing_Images(:, :, i) = enhanceContrastAuto(Testing_Images(:, :, i));
...
%Timing=toc
%saveToText("SVM LS test dataset ", Timing, Accuracy, TP, FP, TN, FN);
...
for i = 1:7
   ...
   %figure
   %imshow(tmpIm);
   tmpIm = enhanceContrastAuto(tmpIm);
...
%Timing=toc;
%saveToText("SVM LS supplied images ", Timing, Accuracy, TP, FP, TN, FN);