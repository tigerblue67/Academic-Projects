...
for i=1:size(Training_Images, 1)
   Edges = edge(Training_Images(i,:),'Prewitt');
   Train_Edge_Images(i, :) = double(Edges);
end
...
for i=1:size(Testing_Images, 1)
    Edges = edge(Testing_Images(i,:),'Prewitt');
    Test_Edge_Images(i, :) = double(Edges);
end
